...
use windows::Win32::Graphics::Dxgi::{
    DXGI_ADAPTER_DESC1, DXGI_ERROR_NOT_FOUND, DXGI_OUTPUT_DESC, IDXGIAdapter1, IDXGIDevice,
    IDXGIDevice1, IDXGIFactory1, IDXGIOutput,
};
...
impl Device {
    ...
    pub(super) fn on(
        ...
        let made = match create(adapter, &[D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0]) {
            // A runtime that does not know 11.1 refuses the list whole.
            Err(e) if e.code() == E_INVALIDARG => create(adapter, &[D3D_FEATURE_LEVEL_11_0]),
            made => made,
        };
        let (device, context) =
...
/// most, and lets encoders use it from their own threads. A refusal
/// costs smoothness, not the session: it is said, and the device used.
fn tune(device: &ID3D11Device, log: &Log) {
    ...
    match device.cast::<ID3D11Multithread>() {
        ...
        Err(e) => log.write(&failed("protecting the device between threads", &e)),
    }
}
