...
Quand le multicast ne traverse pas, et c'est fréquent entre une carte filaire et une carte sans fil derrière la même box, ZyrDesk appelle au lieu d'attendre d'être entendu : un petit port à lui, UDP 47001, sur lequel chaque service répond à qui l'appelle, un datagramme vers l'adresse de diffusion de chaque carte toutes les dix secondes, et tant que personne n'a répondu un passage adresse par adresse du réseau, au plus toutes les trente secondes et seulement jusqu'à 256 adresses (D19). C'est du trafic ordinaire, routé comme une session : un réseau qui porte une session porte cela. Ce qui répond entre dans la même liste que ce qui s'annonce.
