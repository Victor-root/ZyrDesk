...
- Contrôleur de congestion média (section 3), file d'émission de datagrammes taillée sur le profil du flux (six images, 256 Kio au minimum), file de réception de 8 Mio, expiration d'inactivité à 30 s, maintien de correspondance toutes les 5 s.
...
- Contrôleur de congestion média sur mesure : fenêtre = 2 x débit de session x aller-retour + une image entière, et jamais moins d'une demi-seconde de flux ; les signaux de perte ne la réduisent jamais. Le plancher en temps est ce qui manquait : sur un réseau local, les deux premiers termes donnent quelques dizaines de kilo-octets, soit un trentième de seconde de flux, et la fenêtre s'épuise dès que l'ordinateur d'en face cesse de répondre le temps d'un hoquet. Tout ce que l'encodeur produit pendant ce temps s'entasse derrière, puis se jette (D126). Ne pas réagir aux pertes serait déraisonnable pour un flux capable de saturer un lien ; ce n'est pas le cas ici, le débit est fixé par l'encodeur et ne dépasse jamais sa consigne. La fenêtre ne sert donc pas à émettre davantage, seulement à ne pas bloquer ce que l'encodeur produit déjà. Le trafic fiable reste minuscule et ne peut pas être affamé. L'aller-retour retenu est plafonné à 500 ms, pour qu'une mesure aberrante ne produise pas une fenêtre absurde.
