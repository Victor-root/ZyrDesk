...
- File d'émission de datagrammes taillée sur le profil du flux : sous congestion, on JETTE le périmé (la correction d'erreur du protocole l'absorbe) au lieu d'empiler de la latence, mais jamais au point de couper une image en deux. Une image part d'un bloc et la pompe la pousse dans la file bien plus vite que le transport ne la met sur le fil : une file plus courte qu'une image perd des paquets de chaque image clé sur le meilleur des réseaux, et cette perte-là ne se répare pas (D125). Six images, 256 Kio au minimum ; ce que ça coûte sur un chemin vraiment trop lent est borné par ces mêmes six images.
