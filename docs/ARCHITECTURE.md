...
| `zyrdesk-host-engine.exe` | Sunshine (renommé, non modifié), strictement lié à 127.0.0.1 | SYSTEM, dans la session console | Tant que « Autoriser l'accès distant » est actif |
...
Sunshine est lancé par le service avec une configuration générée : liaison sur 127.0.0.1 uniquement, port de base tiré dans la plage 42000 à 42999 (coexistence automatique avec un éventuel Sunshine « normal » déjà installé), interface web verrouillée au PC local avec identifiants aléatoires, icône de zone de notification désactivée, liste d'applications réduite à « Desktop », état et journaux dans `%ProgramData%\ZyrDesk`. Aucune modification de son code : tout passe par son fichier de configuration, sa ligne de commande et son API locale. Détails : [engines/STRATEGY.md](engines/STRATEGY.md).
...
5. Le service client expose des ports locaux factices (adresse loopback stable 127.77.x.y par appareil distant) et lance `zyrdesk-session.exe` vers cette adresse. Pour Moonlight, l'hôte est « local » ; en réalité chaque paquet traverse le tunnel.
...
- Journaux : tous les composants écrivent dans `%ProgramData%\ZyrDesk\logs` (rotation), bundle de diagnostic expurgé en un clic. `zyr-cli doctor` vérifie : encodeurs disponibles, GPU hybride, règle pare-feu, service actif, broker joignable, type de NAT, latence relais.
...
- Interface <-> service (pipe) : `GetStatus`, `Login/Logout`, `EnrollDevice`, `SetHostEnabled`, `Get/SetHostConfig` (écran, GPU, plafonds de qualité, politique de consentement), `ListRemoteDevices`, `Connect(appareil, réglages)`, `ListActiveSessions`, `Disconnect`, presse-papiers, `GetDiagnosticsBundle`, `CheckUpdate/BeginUpdate` ; événements : état de session, statistiques, session entrante, présence, santé moteur.
