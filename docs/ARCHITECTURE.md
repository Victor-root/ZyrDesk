...
- Journaux : tous les composants écrivent dans le sous-dossier `logs` des données du produit (rotation), en temps universel et sous la même forme, le service et la fenêtre partageant le même écrivain. Chaque ligne porte entre crochets le nom de la partie du produit qui l'a écrite, une étiquette par module, et l'écran du journal porte une boîte de tri dans la langue du journal d'un téléphone (`tag:clipboard`, un mot seul, un moins pour exclure) : le tri voyage jusqu'au service et jusqu'à l'ordinateur d'en face, et se fait à la lecture des fichiers avant la coupe. La page se resserre d'elle-même peu après la dernière lettre, et c'est cette page que « Copier » emporte ([D183](DECISIONS.md)). Chaque ligne porte aussi sa voix : ce que le produit dit de lui-même est écrit dans toutes les versions, ce qui compte ou mesure ne l'est que dans la version de débogage, où ce n'est même pas assemblé ailleurs (`level:debug` au tri, et la ligne de version dit de quelle version il s'agit). Détails : [D181](DECISIONS.md) et [D182](DECISIONS.md). Chaque binaire porte l'empreinte du code dont il a été compilé, gravée par un script de compilation, et l'écrit en tête de sa trace : une panne se lit toujours contre la version qui l'a produite. La fenêtre rassemble les quatre traces sur un écran, sous cet entête, avec un bouton qui copie l'ensemble. `zyr-cli doctor` vérifie : encodeurs disponibles, GPU hybride, règle pare-feu, service actif, broker joignable, type de NAT, latence relais.
