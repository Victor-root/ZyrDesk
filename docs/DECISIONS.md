...
**La règle.** Tout ce qui est du code s'écrit en anglais : noms de fichiers et de modules, types, fonctions, variables, constantes, noms des tests, commentaires et documentation du code, jetons et classes de `crates/zyr-ui/design.css`. Les scripts suivent la même règle : installation du serveur, installateur Windows, empaquetage des moteurs, CI. Ce que la personne lit reste en français : les textes de l'interface et les messages qu'elle voit. La règle est redite dans le [README](../README.md), section « Conventions du code », pour qu'une IA qui reprend le dépôt la trouve avant d'écrire une ligne.
