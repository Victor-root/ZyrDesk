...
- Fait : la tranche 1. Le serveur existe sous `server/` : comptes, appareils rattachés par la preuve de leur clé, jetons courts, contacts et partages dans le modèle, canal vivant avec présence et rendez-vous, clair refusé hors de la boucle locale, binaire statique compilé par le flux de travail « Serveur », et `install.sh` qui installe, met à jour, reconfigure ou retire le tout sur un Debian en conteneur LXC, avec trois façons d'arriver au TLS. Côté appareil, le service tient le lien de compte, épingle un certificat auto-signé une fois son empreinte comparée, montre les appareils du compte dans « Mes ordinateurs » avec leur présence, laisse entrer par ticket signé un appareil que le réseau ne lui montre pas, et ouvre une voie vers lui par le rendez-vous, en courant vers les adresses qu'il annonce. La section Compte des réglages rattache, renomme, révoque et détache ; `zyr-cli account` fait de même sans fenêtre. Ce qui manque pour traverser une box : le miroir, le mappage de port et les sondes de la tranche 2 ; aujourd'hui le rendez-vous n'aboutit qu'entre deux machines qui se joignent déjà par une adresse locale (réseau local, VPN). Mode d'emploi du serveur : [server/README.md](../server/README.md) ; à vérifier sur deux PC et un conteneur en suivant [docs/testing/M5-PROTOCOLE.md](testing/M5-PROTOCOLE.md).
