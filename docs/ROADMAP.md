...
- Fait : le service s'installe, tient son moteur et le relance selon une politique qui distingue un arrêt système d'un incident ; il démarrait alors avec Windows, ce qui est devenu un réglage au jalon M4 ([D20](DECISIONS.md)) ; le moteur est lancé dans la session attachée à l'écran, avec le jeton du service déplacé vers cette session, et il est relancé quand cette session change.
...
- Fait : plus rien à taper entre deux ordinateurs du réseau local. Le service admet les empreintes que le réseau annonce, sous un interrupteur activé par défaut, et le code d'appairage des moteurs voyage dans le tunnel au lieu de s'afficher. Le service s'installe et se démarre depuis la fenêtre, par la seule élévation que le produit demandera jamais. Décision [D17](DECISIONS.md).
- Fait : le service ne s'arrête plus quand le moteur hôte manque, et ce qui empêche d'être joignable se lit dans la fenêtre au lieu d'un « démarrage en cours » sans fin. Un ordinateur sans moteur hôte reste un client à part entière. Décision [D18](DECISIONS.md).
- Fait : le journal dans la fenêtre. Tout ce que les quatre composants écrivent, rassemblé sous la compilation qui l'a produit, avec un bouton qui copie l'ensemble. Chaque binaire porte son commit, gravé à la compilation, et la fenêtre montre celui du service à côté du sien : deux moitiés compilées à des dates différentes est la panne que personne ne pense à vérifier.
- Fait : une session n'ouvre plus de deuxième fenêtre. Le moteur client dessine toujours dans une fenêtre à lui, ce qui est indispensable pour que rien de ZyrDesk ne soit sur le chemin d'une image, mais cette fenêtre est dépouillée de son cadre, posée sur l'intérieur de la nôtre et contrainte à la suivre. Le réglage d'affichage ne parle plus que de notre fenêtre, plein écran ou fenêtre, et le plein écran exclusif du moteur disparaît avec. Décision [D21](DECISIONS.md).
- Fait : plus une seule bande noire. L'ordinateur d'en face met son bureau à la forme demandée pendant la session et le remet après, ce qui retire les bandes que le moteur hôte gravait dans chaque image pour faire entrer un écran d'une forme dans un flux d'une autre ; et notre fenêtre prend la forme de l'image au lieu de la lui imposer, ce qui retire celles que le moteur client ajoutait à l'autre bout. Un moniteur 4K regardant un portable seize-dixièmes remplit maintenant l'écran. Décision [D22](DECISIONS.md).
- Fait : les deux fenêtres d'une session n'en font plus qu'une aux yeux du système. La barre de titre reste allumée quand c'est notre propre image qui tient le premier plan, le bouton flottant est possédé par la fenêtre d'accueil comme l'image et se repose avec elle à chaque déplacement, et la croix termine la session au lieu de laisser tourner quelque chose que plus rien à l'écran ne permet de lâcher. La session retient aussi comment elle a été laissée, plein écran ou fenêtre. Décision [D23](DECISIONS.md).
- Fait : une session est en cours ou terminée. Les moteurs distinguent partir, qui laisse le bureau distant ouvert et en attente, et fermer, qui le rend ; cette distinction a été portée jusqu'à la personne et c'était une erreur. Une seule entrée reste, et elle rend le bureau. Décision [D24](DECISIONS.md).
...
- Fait : la tranche 1. Le serveur existe sous `server/` : comptes, appareils rattachés par la preuve de leur clé, jetons courts, contacts et partages dans le modèle, canal vivant avec présence et rendez-vous, clair refusé hors de la boucle locale, binaire statique compilé par le flux de travail « Serveur », et `install.sh` qui installe, met à jour, reconfigure ou retire le tout sur un Debian en conteneur LXC, avec trois façons d'arriver au TLS. Côté appareil, le service tient le lien de compte, épingle un certificat auto-signé une fois son empreinte comparée, montre les appareils du compte dans « Mes ordinateurs » avec leur présence, laisse entrer par ticket signé un appareil que le réseau ne lui montre pas, et ouvre une voie vers lui par le rendez-vous, en courant vers les adresses qu'il annonce. La section Compte des réglages rattache, renomme, révoque et détache ; `zyr-cli account` fait de même sans fenêtre. Mode d'emploi du serveur : [server/README.md](../server/README.md) ; à vérifier sur deux PC et un conteneur en suivant [docs/testing/M5-PROTOCOLE.md](testing/M5-PROTOCOLE.md).
- Fait : la tranche 2, moins le mappage de port. Le transport parle par un aiguilleur : une prise à nous sous QUIC, où chaque ordinateur présenté par le serveur reçoit une adresse de carte fixe, et où des sondes signées par la clé d'appareil essaient toutes les adresses que les deux côtés nomment, valident celles qui répondent, mesurent leur aller-retour et élisent la plus courte ; un chemin qui change est une ligne dans une table, et la connexion ne voit rien. Les candidats voyagent par le canal vivant au fur et à mesure : les adresses locales, IPv6 globales comprises, puis l'adresse vue par le miroir du serveur, un port UDP sans état qui répond « tu m'écris de là », demandé depuis la prise même du tunnel ; et la prise parle les deux versions d'IP. Les deux box s'ouvrent par la perforation, ou par un port renvoyé à la main chez l'hôte, que le miroir laisse deviner. Le serveur se vérifie aussi par son miroir, et l'accueil dit par où passe une session et combien la route prend. Reste de la tranche : le mappage de port UPnP, NAT-PMP ou PCP chez l'hôte, qui vaudra pour les box qui changent de port à chaque destination ; le chemin dans le menu de la session viendra avec le relais, quand il y aura deux chemins à distinguer.
