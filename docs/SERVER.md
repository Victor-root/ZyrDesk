...
Ce document conçoit ce qui manque encore à ZyrDesk pour se joindre à travers Internet sans rien recopier : un serveur facultatif qui tient les comptes, dit qui est en ligne, met deux ordinateurs en relation, et transporte en dernier recours des paquets qu'il ne peut pas lire. Il complète [NETWORK.md](NETWORK.md), qui décrit le tunnel, et [SECURITY.md](SECURITY.md), qui décrit les identités. C'est l'étude, écrite avant de coder et validée avant le jalon M5 ([ROADMAP.md](ROADMAP.md)) ; la première des tranches du §12 est livrée depuis, et ce qu'elle a fait autrement est noté en place. Les décisions qu'elle prend sont consignées de D118 à D123 dans [DECISIONS.md](DECISIONS.md), et les questions qu'elle laisse à Victor sont rassemblées au §13.
...
5. Le tunnel actuel ne change pas : la migration se fait **sous** lui, dans une couche qui choisit le chemin, de sorte que ni le chiffrement, ni le contrôle de congestion, ni le moteur ne s'en aperçoivent. On reste sur quinn.
...
- **Réseau local.** Les deux services s'annoncent en mDNS et s'appellent directement en UDP quand le multicast ne passe pas (D19) ; le service hôte admet ce qui s'annonce sous l'interrupteur de confiance au réseau local (D17) ; les deux ordinateurs se reconnaissent à leurs empreintes, sans aucun code. Rien à recopier.
- **VPN, adresse publique avec port ouvert, deux machines réglées à la main.** Le chemin existe déjà, c'est l'ajout par empreinte : sur le PC qui regarde, « Ajouter un ordinateur » avec l'empreinte de l'hôte et son adresse ; sur l'hôte, la même carte avec l'empreinte du PC qui regarde. L'adresse peut être celle d'un VPN, ou une adresse publique derrière laquelle la box de l'hôte renvoie le port **UDP 47000** vers lui. Un seul port, et rien d'autre à ouvrir : le moteur n'écoute sur aucun port, il parle au service de sa machine par un tube local ([MOTEUR.md](MOTEUR.md) §2). La liste des appareils admis est relue toutes les cinq secondes, donc rien à redémarrer.
...
- Chaque ordinateur d'en face reçoit une **adresse de carte**, stable et fictive, tirée de son empreinte. C'est à cette adresse que quinn croit parler. Prévue dans un préfixe IPv6 privé, elle vit finalement dans `240.0.0.0/4`, le bloc IPv4 que personne ne route et qu'aucune carte réseau ne porte : le transport refuse une adresse IPv6 sur une prise qui ne parle qu'IPv4, et une carte IPv4 passe partout, sur une prise à double pile comme sur une machine où IPv6 est coupé.
...
Une taille de paquet, et une seule : QUIC exige que tout chemin porte 1200 octets, et le moteur hôte reçoit du service, avant sa première image, la taille de datagramme que ce plancher garantit, moins notre octet de canal, et la garde pour toute la session ([NETWORK.md](NETWORK.md) §4). La découverte de MTU est donc **désactivée** sur le point d'accès qui passe par l'aiguilleur, et chaque paquet du tunnel fait au plus 1200 octets, ce qui tient dans un datagramme du relais (§4.5). C'est quelques dizaines d'octets de moins par paquet qu'un chemin direct ordinaire n'en permettrait, et c'est le prix d'une bascule sans que personne s'en aperçoive.
...
6. Le service client ouvre la connexion QUIC vers l'adresse de carte de l'hôte, **tout de suite**, sans attendre qu'un chemin soit validé : l'aiguilleur l'envoie par la branche de relais dès qu'elle est prête, ou par le premier chemin direct validé si c'est lui qui arrive d'abord, ce qui est le cas courant avec une box ordinaire (une sonde et son écho prennent un aller-retour ; la branche de relais deux). Les quelques paquets partis avant qu'aucun chemin n'existe, ceux de la poignée de main, sont gardés et partent d'un coup au premier chemin qui répond, plutôt que d'attendre que QUIC les réémette une seconde plus tard.
7. À partir de là, tout est comme aujourd'hui : première question du canal ZyrDesk qui prouve l'autorisation, écran d'en face, ouverture de la session avec le moteur hôte lancé pour elle, lecteur branché sur sa voie par un tube local. Rien de ce chemin ne sait qu'un serveur a servi.
...
- **Limites et quotas**, parce que le port du relais est la seule chose du serveur que n'importe qui peut atteindre sans compte : un plafond de nouvelles connexions par adresse, un plafond de débit par session relayée (60 Mb/s par défaut, réglable) au-delà duquel les datagrammes sont jetés, un nombre maximal de sessions relayées en même temps, et le compte des octets par session, écrit dans la base à la fin pour les quotas par compte à venir. Le relais ne garde rien d'autre.
- **Ce que les deux appareils s'échangent à côté de leur tunnel a son propre petit seau** : les sondes signées de l'aiguilleur passent par le relais comme le reste, et ce sont elles qui mesurent le chemin relayé et le tiennent vivant. Elles ne peuvent donc pas être jetées parce que l'image est à son plafond, ce qui tuerait le chemin même qu'elles portent ; elles ne comptent pas non plus comme du trafic de session, sans quoi une session partie en direct compterait comme relayée. Un centième du débit de la session leur est réservé, cent fois ce qu'il leur faut. Les distinguer coûte quatre octets lus en tête de datagramme, la marque qu'elles portent ; le tunnel, lui, reste du chiffré que le relais ne regarde pas.
...
- **Mes ordinateurs** : tous les appareils du compte, cet ordinateur-ci compris, avec leur état : en ligne et joignable, en ligne sans accès distant (avec la raison, que le service connaît déjà : FFmpeg absent, accès coupé), hors ligne depuis telle heure. Renommer et révoquer se font ici.
...
- **Briques** : axum 0.8 pour HTTPS et WebSocket, rustls 0.23 (certificat lu en PEM), rusqlite avec SQLite embarqué en mode WAL et des migrations numérotées, tenues par le serveur lui-même dans le `user_version` de la base, `argon2` 0.6, `ed25519-dalek` 3, et `zyr-transport` pour le relais et le miroir, qui tiennent le même port UDP : quinn n'est nommé que là, jamais ici. Un fichier de base, sauvegardable en une commande ; Postgres reste une possibilité quand un besoin réel le demandera ([TECH-CHOICES.md](TECH-CHOICES.md)).
...
- **Données** : `zyrdesk.db` (comptes, appareils, jetons hachés, contacts, partages, journal des sessions : identifiants, dates, chemin, octets relayés, gardé 30 jours), `keys/signing.key`, et `keys/relay.crt` avec `keys/relay.key`, que le relais se fait à son premier démarrage. Propriété `zyrdesk:zyrdesk`, mode 700.
- **Ligne de commande d'administration**, sur la machine, sans passer par l'API : `zyrdesk-server status` (comptes, appareils, et ce que le relais a porté en trente jours), `user create|list|reset-password|delete`, `invite new|list|revoke`, `fingerprint` (l'empreinte à comparer dans l'application), `check` (le serveur se joint lui-même comme un appareil le ferait, avec la même confiance TLS : l'API, le miroir, et le nom auquel les appareils sont envoyés pour le relais), `backup <dossier>` (copie cohérente de la base par `VACUUM INTO`, plus la configuration et les clés).
...
Cibles : Debian 12 (support LTS jusqu'en juin 2028) et Debian 13 (13.6 est la version stable), en conteneur non privilégié sur Proxmox VE 8 ou 9 (9.2 est la version courante), ou sur toute machine Debian ordinaire. Binaire publié en x86_64, l'architecture des conteneurs de Proxmox ; une autre architecture compile sur place par `--from-source`, ce qui prend quelques minutes pour ce seul binaire.
...
Rien dans le moteur, et c'est vérifié par construction : tout ce qui suit vit sous le tunnel, dans le transport et le service, et le moteur ne voit jamais que son tube local.

- **`zyr-transport`** : l'aiguilleur (la prise virtuelle, les adresses de carte, la table des chemins, l'élection) ; les sondes (dialecte, signature, vérification, rythmes) ; la branche de relais des deux côtés (connexion QUIC extérieure, laissez-passer, datagrammes) et la porte sur laquelle un serveur pose son relais, miroir compris ; le miroir côté client ; la prise en double pile IPv4 et IPv6. quinn n'est nommé que là, dans `endpoint.rs`, `junction.rs`, `relay.rs` et `sifting.rs`, et nulle part ailleurs dans le produit ; le point d'accès posé sur un aiguilleur y fige sa taille de paquet. C'est aussi là que vit la confiance TLS vers le serveur (`trust` : vérification publique, puis épinglage de la clé), parce que le service l'emploie pour se rattacher et que le serveur l'emploie pour se joindre lui-même.
...
- **`zyrdeskd`** : tenir le lien de compte quand il existe ; alimenter `let_in` d'un troisième ensemble, les empreintes admises par ticket avec leur expiration ; fournir les candidats à l'aiguilleur ; dire au broker l'état de l'accès distant ; ouvrir une voie vers un appareil du compte par le rendez-vous. La porte tient son aiguilleur dans la machine, pour que le compte y fasse attendre l'ordinateur présenté ; le registre des voies gagne le chemin en cours et l'écrit dans `sessions`.
- **`zyr-control`** : des demandes de plus (`account`, `attach`, `detach`, `devices`, `rename-device`, `revoke-device`, puis `contacts` et `shares` avec la tranche 3, les listes se terminant par `done`), une provenance de plus sur `Peer` (`device`, `online`, `access`, `shared`), le chemin sur `Session` avec la tranche 2, et le numéro de protocole qui monte.
...
- **`zyr-cli`** : `account status|attach|detach|devices|rename|revoke` pour diagnostiquer sans fenêtre, `net observe` pour lire son adresse vue par un miroir (tranche 2).
...
**Où on en est.** La tranche 1 est livrée : le serveur, son script, son flux de travail et son mode d'emploi ([server/README.md](../server/README.md)) ; côté appareil, le lien de compte tenu par le service, la section Compte, les cartes du compte dans « Mes ordinateurs », la fenêtre d'épinglage, et `zyr-cli account`. La tranche 2 l'est aussi, à une pièce près : l'aiguilleur (§4.1), les sondes (§4.3), le miroir et les candidats qui voyagent au fur et à mesure (§4.2, §4.4), la prise en double pile, et le serveur qui se vérifie par son miroir. Ce qui a été fait autrement que prévu est dit en place : les cartes vivent en `240.0.0.0/4` et non dans un préfixe IPv6, les paquets partis avant tout chemin attendent le premier au lieu d'être perdus, et le miroir ne signe pas ses réponses. Il manque le mappage de port chez l'hôte (UPnP, NAT-PMP, PCP), qui viendra en complément : sans lui, deux box qui changent de port à chaque destination ne se joignent que par un port renvoyé à la main, ou par le relais. Ce que Victor doit vérifier est écrit dans [testing/M5-PROTOCOLE.md](testing/M5-PROTOCOLE.md), et la session par Internet entre deux fibres différentes a été tenue.

**M6, le relais et la bascule**, livré : la branche de relais des deux côtés (§4.5), les laissez-passer (§3.7), les quotas, la bascule dans les deux sens (§4.6) et le chemin affiché. Critère : UDP direct bloqué entre les deux PC, la session s'établit quand même ; on débloque, elle passe en direct sans coupure perceptible ; surcoût du relais d'au plus un aller-retour vers lui. Ce que Victor doit vérifier est écrit dans [testing/M6-PROTOCOLE.md](testing/M6-PROTOCOLE.md).

Ce qui a été fait autrement que la conception ne le disait, et qui vaut d'être su :

- **Le relais mesure comme n'importe quelle route.** Les sondes signées de l'aiguilleur passent aussi par la branche de relais, donc le chemin relayé est mesuré de bout en bout et meurt au bout de trois sondes sans écho, exactement comme un chemin direct. L'écho qui revient par un relais ne porte pas d'adresse vue : un relais n'en connaît aucune, et le bout d'en face ignore ce champ-là.
- **Le laissez-passer ne se consomme pas.** Un ticket vaut une fois ; un laissez-passer vaut tant qu'il vit, et c'est ce que ses cinq minutes servaient déjà à dire (§3.7) : un appareil dont la connexion au relais a lâché revient avec le même. Ce qui empêche un autre de s'en servir n'est pas une mémoire tenue au relais, c'est le certificat : le laissez-passer nomme son porteur, et présenter cette empreinte veut dire détenir cette clé.
- **Le paquet du tunnel est figé à 1200 octets sur un aiguilleur.** La découverte de taille est coupée là (§4.1 le prévoyait), et pour la raison qui saute aux yeux dès qu'il y a deux routes : elle aurait trouvé ce que porte celle du moment, et la suivante aurait reçu des paquets qu'elle ne prend pas. La branche vers le relais, elle, part de 1280 et découvre au-dessus, parce qu'il lui faut porter ces 1200 octets plus son enveloppe.
- **Les quotas se règlent dans `[relay]`**, `connections_per_minute` compris, et non dans `[limits]` : ce sont les limites du relais, elles vivent avec lui.
- **Une session relayée qui n'a rien porté n'est pas écrite dans la base.** Les deux appareils ouvrent leur branche à chaque session, direct compris ; dans le cas ordinaire un chemin direct répond le premier et le relais tient une route que rien n'emprunte, sondes mises à part. Écrire cette route-là aurait fait de chaque session une session relayée, et le compteur du serveur n'aurait plus rien dit.
