...
**Où on en est.** La tranche 1 est livrée : le serveur, son script, son flux de travail et son mode d'emploi ([server/README.md](../server/README.md)) ; côté appareil, le lien de compte tenu par le service, la section Compte, les cartes du compte dans « Mes ordinateurs », la fenêtre d'épinglage, et `zyr-cli account`. Le rendez-vous existe déjà, ticket signé et admission par empreinte compris, mais ses seuls candidats sont pour l'instant les adresses locales de l'hôte : sans le miroir et le mappage de port de la tranche 2, il n'aboutit qu'entre deux machines qui se joignent déjà par une adresse locale. Le serveur annonce donc « pas de relais », et son `check` ne joint que l'API. Ce que Victor doit vérifier de la tranche 1 est écrit dans [testing/M5-PROTOCOLE.md](testing/M5-PROTOCOLE.md).
