...
>
> **Et ce qui répond vraiment : le bouton ne change plus de taille.** Sa boîte fait en permanence la plus grande des trois tailles et c'est l'image dedans qui grandit et rétrécit. La forme envoyée au coeur ne bouge donc plus pendant l'animation, et le coeur ne redécoupant que quand la forme change, **il ne touche plus du tout à la découpe** entre l'ouverture et la fermeture du menu. On avait essayé de synchroniser une découpe animée avec ce que la page peignait ; le rendu du navigateur, l'appel vers le coeur et la composition de Windows sont trois files d'attente indépendantes qu'aucun décalage d'une image ne met d'accord. Il fallait cesser d'animer.
>
> **Ce que ça coûte, et c'est le cas à regarder en premier** : la découpe est celle du plus grand des trois dessins, donc au repos elle dépasse de deux ou trois pixels en bas et à gauche. Ces pixels-là sont censés ne rien montrer, la fenêtre étant transparente. **Si un liseré pâle apparaît maintenant en permanence au lieu de faire un éclair, c'est la réponse** : ces pixels ne sont pas transparents, la marge est le défaut, et il faut revenir en arrière sur cette moitié-là.
>
