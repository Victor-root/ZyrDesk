...
| **S9sexies** | **Deux défauts distincts démêlés, les deux corrigés.** D'abord la frappe n'arrivait plus après le menu : le crochet de ZyrDesk s'était fait doubler par un plus récent, il est reposé et les frappes reviennent (`Tab X et X` équilibré). Ensuite, ce qui restait : fermer le menu fait rebondir le premier plan une fraction de seconde sur le shell, et l'Alt+Tab tombé pile là ouvrait le sélecteur local, qui bloquait tout le reste. Un premier plan parti moins d'une demi-seconde n'est plus compté comme perdu |
