...
>
> **Et au rallumage, la réponse tient en une ligne.** `service.log` porte, quelques secondes après le démarrage : `screens the engine sees: … (…, on at 3840x2160)`. C'est la taille réelle de chaque écran vu par le moteur au moment où il démarre. Si elle est bonne, l'écran est revenu ; si elle est encore celle de la session, il ne l'est pas, et cette fois on le sait sans avoir à se planter devant la machine.
