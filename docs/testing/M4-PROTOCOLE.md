...
>
> **La croix par-dessus le bouton, et ce qu'elle est vraiment.** La capture est gardée à [`captures/bouton-flottant-croix.png`](captures/bouton-flottant-croix.png) avec sa mesure complète. Ce n'est pas l'écran d'en face vu au travers : la chose claire s'arrête pile au bord du logo alors que le noir tout autour est pur, donc elle est **dans la fenêtre du bouton, sous la page**. Le logo retiré par le calcul, il reste une **barre de titre avec son bouton de fermeture**, peinte par le système dans cette fenêtre à sa naissance, que plus rien ne couvre depuis que la fenêtre est vraiment transparente. Le fondu du bouton au repos est retiré : opaque, il la couvre entièrement.
>
...
>
> **La cause du flash, trouvée sur ces photos et corrigée.** Ouvrir le menu élargit cette fenêtre de dix-huit pixels, et la découpe était faite contre la largeur que la fenêtre allait avoir alors que la page avait mesuré dans celle qu'elle avait encore. Toute la forme se posait donc dix-huit pixels à droite de ce qui était peint. On découpe maintenant contre la taille que la fenêtre **a**, et la page redemande à l'image suivante, mise en page à la nouvelle largeur. La photo est gardée à [`captures/bouton-flottant-decoupe-decalee.png`](captures/bouton-flottant-decoupe-decalee.png).
>
> **L'appareil photo du bouton a été retiré.** Il s'est photographié lui-même pendant une soirée, d'abord en se redessinant, ce qui ne montre qu'une page juste, puis en recopiant l'écran. Trente-cinq photos, dont une quinzaine prises pile au bon instant, ont toutes montré un bouton irréprochable pendant que l'oeil voyait le défaut, parce que **ce qu'un compositeur ajoute au moment de composer n'est pas dans ce qu'on recopie de l'écran**. Un instrument aveugle au défaut qu'il vise, qui coûtait une copie de trois cent vingt pixels de côté sur le fil qui dessine à chaque agrandissement de la découpe, n'a rien à faire dans un produit. Pour cette famille de défauts, c'est une capture d'écran de Victor qui fait foi.
>
> **Ce qui a été mesuré sur l'image que Victor a filmée**, gardée à [`captures/bouton-flottant-liseret-gauche.png`](captures/bouton-flottant-liseret-gauche.png). Deux choses s'y voient et une seule est un défaut. La tache blanche ronde sur le coin bas droit de l'écran or est **le pointeur de la souris**, la main fermée que la page demande pendant qu'on tient le logo ; aucune copie de fenêtre ne la prend, d'où deux archives où elle manquait. Le défaut est **un liseré pâle large de deux pixels le long du bord gauche, dehors**, à 203,209,216, là où la même marge vaut 5,8,15 en haut, 6,9,15 à droite et 8,11,15 en bas.
>
> **Et la cause, trouvée dans le journal de PC-VICTOR.** En plein survol, la découpe passe de `(1328, 2, 1403, 76)` à `(1326, 2, 1403, 78)` pendant que le dessin passe de 77 à 78 pixels de large : elle gagne deux pixels à gauche là où le dessin en gagne un, donc elle est **posée une image trop tôt**. Une mesure prise dans une image n'y est pas encore peinte, et une découpe en avance découvre une bande de fenêtre que la page n'a pas peinte, à gauche puisque le logo grandit vers la gauche. La page pose maintenant la mesure de **l'image d'avant**, celle que l'écran montre.
>
> **Le cas qui dit si c'est réglé** : survoler le logo, puis cliquer et relâcher, et regarder son bord gauche. Rien de pâle ne doit apparaître, même une image. C'est au **relâchement** qu'il faut regarder : appuyer rétrécit le logo, relâcher le fait regrandir, et c'est grandir qui découvrait la bande.
>
> **Et la réponse, obtenue en rendant le défaut permanent.** La découpe a été agrandie exprès de trois pixels, et le liseré est passé d'un clignotement de deux pixels au clic à une bande permanente de quatre le long de tout le bord gauche. La capture est gardée à [`captures/bouton-flottant-marge-non-peinte.png`](captures/bouton-flottant-marge-non-peinte.png) : fond brun `49,39,29`, puis `215,228,241` quatre fois, puis le contour du dessin. Sur fond noir la même bande valait `203,209,216` : **elle n'a pas de couleur à elle, elle éclaircit ce qu'il y a derrière**. C'est du verre dépoli, celui que la boîte à outils allume pour la transparence par pixel, et il se voit partout où la page ne peint rien.
>
> **Un pixel de fenêtre que la page n'a pas peint n'est pas vide.** La découpe prenait un pixel de plus que le dessin sur les quatre bords, par prudence, et ce pixel était le défaut. Elle n'en prend plus aucun : le bord est toujours arrondi vers le dehors, donc elle contient le pixel qui **contient** le bord, et celui-là est peint en partie par le lissage. Vérifié sur les nombres réels des deux machines : les cinq marges restent positives, la plus petite à cinq centièmes de pixel.
>
> **Ce qu'il faut regarder maintenant** : le liseré doit avoir disparu, au repos comme au clic. Et le contour du logo doit rester lisse et de la même épaisseur des quatre côtés, y compris sur un fond blanc : si un tour en escalier revient, c'est que la découpe mord dans le lissage et le journal le dira, une des cinq marges passant négative.
>
> **Le bouton qui se téléporte et qui revient, corrigé.** Le menu s'ouvre vers le bas ou vers le haut selon la place, et le logo ne doit pas bouger : il est donc en haut de la fenêtre dans un cas et en bas dans l'autre, et **basculer déplace la fenêtre de toute sa hauteur moins le logo**, neuf cents pixels. La page et le coeur ne peuvent pas faire ce demi-tour dans la même image, donc celle qui redessine la première montre le logo à l'autre bout de l'écran avant que l'autre ne rattrape. Ce qui déclenchait la bascule était **le survol** : le logo grandit de six pour cent, la fenêtre gagne trois pixels de hauteur, et trois pixels suffisent à changer la réponse à « est-ce que le menu tient en dessous ».
>
> Le bouton garde donc en permanence la plus grande de ses trois tailles et c'est le dessin dedans qui respire : la mise en page ne bouge plus, la fenêtre ne change plus de hauteur, le sens ne bascule plus tout seul. **À vérifier** : passer la souris sur le bouton posé près du bas de l'image, plusieurs fois, sans cliquer. Il ne doit pas sauter. Et le journal ne doit plus alterner `dessiné vers le bas` et `dessiné vers le haut` d'une ligne à l'autre.

>
> **Le saut de dix-huit pixels, vu sur la photo 8 et corrigé.** À la première ouverture du menu, le bouton entier était dessiné dix-huit pixels à gauche de sa place pendant une image. La barre des mesures était bâtie à cette première ouverture et c'est elle qui décide de la largeur de la carte, donc la fenêtre s'élargissait à cet instant ; accrochée par son coin haut droit, l'élargir déplace son bord gauche, et la vue web garde une image son dessin d'avant. La barre est maintenant posée dès le chargement, remplie de tirets. **À vérifier** : le tout premier clic de la session ne doit pas faire sauter le logo, et le journal ne doit plus contenir de ligne `… demandés, … avant, … après` où la largeur change après le démarrage.
>
...
>
> **Et le test qui sépare le net du mou** : regarder le bouton la souris **loin de lui**, puis la souris **posée dessus**, puis **bouton enfoncé**. C'est le repos qui doit être irréprochable, parce que c'est là que le bouton passe sa vie ; sous la souris il est un peu plus mou, et **c'est su, mesuré et assumé**. À 125 %, 52 % des bords droits du contour sont un noir franc au repos, contre 2 % agrandi de six pour cent : le dessin est vectoriel mais l'écran est une grille, et 55 pixels réels sur les 440 du dessin tombent juste, un huitième, là où 58,3 tombent n'importe où. Le grandissement a été retiré une fois pour cette raison et remis aussitôt : Victor le veut, et c'est son bouton.
>
...
>
> **Et ce que le survol fait sans qu'on le lui demande, à savoir redessiner.** Le cœur ne redécoupe que quand la forme change, et une découpe est le seul moment où le système redessine cette fenêtre. Le survol en déclenche dix par seconde, donc il balaye tout ce qui traîne. Sans lui, plus rien ne redessine entre deux ouvertures du menu : **si un artefact apparaît un jour sur ce bouton et qu'il ne part plus, c'est ici qu'il faut revenir**, et non sur la découpe elle-même.
>
...
> **R67 (le logo du bouton flottant, dessiné par ZyrDesk)**
>
> **Réglé.** Le liseré blanc a disparu avec la vue web, confirmé sur PC-VICTOR. C'était bien elle : ce qu'on voyait n'était pas un liseré mais le fond blanc d'un navigateur, laissé une image à l'écran chaque fois qu'il refaisait sa surface, et la découpe collée au dessin n'en laissait passer qu'un trait de deux pixels. Ce qui suit reste à vérifier une fois, parce que tout ce qui touche ce bouton a été réécrit.
>
> **Ce qui a changé.** Le logo n'est plus une image dans une page web : c'est une fenêtre à lui, dont ZyrDesk calcule chaque pixel, transparence comprise, et qu'il remet telle quelle à Windows. Le menu, lui, reste une page web, parce que c'est de la vraie interface.
>
> **Pourquoi.** Onze essais, onze fois non : ni la découpe, ni le redessin, ni le calque, ni le fond, ni la transparence, ni le message de redimensionnement, ni le déplacement. La page a été mesurée hors de cause, au repos et en pleine animation. La vue web était la seule couche jamais éteinte, et la seule dont le fond à elle est blanc.
>
> **Ce que ça enlève, en plus du navigateur** : il n'y a plus de forme à découper, la forme étant la transparence de l'image ; plus de fond à effacer ; plus de cadre ; et plus de clics à laisser passer, le système les laissant déjà passer là où l'image est claire.
>
> **La ligne qui dit que c'est bien le nouveau** :
>
> ```
> bouton flottant : logo dessiné par ZyrDesk, sans vue web
> ```
>
> **Ce qu'il faut vérifier, et c'est tout le bouton d'un coup**
>
> **Le liseré, qui est la raison de tout ça.** Poser le bouton sur une fenêtre bien blanche du bureau distant, puis sur du bien noir. Cliquer dessus plusieurs fois. Le prendre et le traîner d'un bout à l'autre de l'image. **Aucun blanc nulle part** : ni liseré au bord, ni carré derrière le logo, ni éclair au clic.
>
> **Le bord lui-même.** Le contour noir doit être lisse et de la même épaisseur des quatre côtés, sur fond blanc comme sur fond noir. Il n'y a plus de pochoir à un bit par pixel nulle part, donc plus rien qui puisse faire un escalier.
>
> **Le grandissement au survol.** Passer la souris sur le logo sans cliquer : il grandit de six pour cent, doucement, et redescend quand la souris s'en va. L'appuyer : il rétrécit. C'est calculé à chaque taille et non étiré, donc il doit rester net partout.
>
> **Le déplacement.** Le prendre et le poser ailleurs. Il suit la main sans traîner, il reste où on le pose, et il ne saute pas. Le curseur devient une main au survol et une croix de déplacement pendant qu'on le tient.
>
> **Le menu.** Un clic simple l'ouvre, un autre le ferme. Il s'ouvre exactement où il s'ouvrait avant, la place du logo ayant été gardée dans la page. Vers le haut quand le bouton est posé en bas de l'image. Toutes ses entrées marchent.
>
> **Ce qui n'attrape plus les clics.** Menu fermé, tout ce qui n'est pas le logo doit laisser passer les clics jusqu'à l'image, y compris le vide entre les deux écrans du logo.
>
> **Le masquer et le rappeler.** L'entrée qui range le bouton doit ranger le logo avec le menu, et le raccourci clavier doit ramener les deux.
>
...
>
> Rien de neuf à voir : ce qu'il faut vérifier est que **rien n'a bougé**, dans les endroits que la boîte à outils tenait.
>
> **La fenêtre.** Elle s'ouvre au milieu de l'écran, à la bonne taille, dans le bon thème, déjà peinte. Elle se redimensionne, s'agrandit, se réduit et revient. Elle ne descend pas en dessous de 880 x 600. La croix la range ; l'icône près de l'horloge la ramène ; un second ZyrDesk lancé la ramène aussi au lieu d'ouvrir une deuxième fenêtre.
>
> **Sur deux écrans d'agrandissements différents.** Faites-la passer de l'un à l'autre : tout ce qu'elle dessine doit se recompter, y compris le texte des trois champs de saisie, et rien ne doit devenir flou.
>
> **L'icône près de l'horloge.** Nette à votre agrandissement d'écran. Vive quand cet ordinateur est joignable, en retrait sinon, et son mot au survol suit. Clic gauche : la fenêtre revient. Clic droit : « Ouvrir ZyrDesk » et « Quitter ». Quitter arrête le service **et fait disparaître l'icône tout de suite**, sans laisser de fantôme qu'un passage de souris efface.
>
> **Et tout le tour de la session, encore une fois**, parce que c'est la fenêtre qui a changé de main : ouvrir, plein écran, revenir en fenêtre, tirer un bord pendant que l'image est dedans, Alt+Tab, la croix qui termine, le bouton flottant qui suit.

> **R72 (l'accueil dessiné par ZyrDesk)**
>
...
>
> **Mes ordinateurs.** Les cartes apparaissent seules, le journal de chacune s'ouvre par son coin, un clic n'importe où ailleurs se connecte. La tuile « + » ouvre le dialogue d'ajout.
>
> **Ajouter un ordinateur.** Les trois champs se remplissent, se collent au presse-papiers, se parcourent à la tabulation. Le compte de caractères sous l'empreinte suit la frappe. Entrée vaut « Se connecter », Échap ferme. « Oublier » retire.
>
> **Le journal.** Le sien et celui d'en face. Il s'ouvre par le bas, défile à la roulette, en travers avec Maj. « Vider » demande une confirmation qui retombe toute seule. « Copier tout » copie tout.
>
> **Les réglages.** Les trois interrupteurs, les quatre choix segmentés, le repli « Avancé », les trois raccourcis qui s'attrapent en tapant la combinaison, Échap qui annule, Retour arrière qui retire. Le thème bascule tout de suite, barre de titre comprise, et il est retenu au prochain démarrage. Et : basculez le thème de **Windows** pendant que la fenêtre est ouverte, elle doit suivre.
>
> **L'ouverture d'une session.** L'écran couvre la fenêtre, la barre va et vient, les étapes s'écrivent dessous, le code d'appairage se lit de loin. À la fin, l'accueil revient et l'image prend sa place.
>
> **Et le tour du bouton flottant, encore** : rien de ce qui le concerne n'a changé, mais il vit maintenant dans une fenêtre dont le dedans a changé de main.

> **R71 (plus de vue web sur l'image)**
>
> Il n'y a plus qu'un menu, et il s'ouvre au **clic gauche**. Un clic simple ouvre, un autre referme ; prendre le logo et le traîner le déplace sans rien ouvrir, avec le rétrécissement à la prise, comme avant.
>
> **Le tour complet du bouton, une fois**, parce que tout ce qui l'entoure a été réécrit :
>
> 1. **Rien de blanc nulle part**, sur fond clair comme sur fond sombre, au repos, au clic et pendant qu'on le traîne.
> 2. **Rien n'attrape les clics** en dehors du logo et de la carte ouverte : cliquer entre les deux écrans du logo doit atteindre l'image.
> 3. **Le déplacement** : il suit la main sans traîner, reste où on le pose, et le retrouve à la session suivante.
> 4. **Le sens d'ouverture** : posé en bas de l'image, le menu s'ouvre vers le haut, et le logo ne bouge pas d'un pixel.
> 5. **Masquer, puis le raccourci** : les deux fenêtres partent ensemble et reviennent ensemble, menu ouvert.
