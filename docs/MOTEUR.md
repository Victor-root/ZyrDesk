...
2. **Cadence.** Une image capturée part tout de suite, sauf si elle arrive plus vite que la cadence de la session (alors seule la plus récente part). Quand l'écran ne bouge plus et que « Fluide » est choisi, l'image précédente est renvoyée à la cadence exacte de la session, pour que le curseur et la qualité s'affinent ; une vraie image ne cède jamais sa place à une répétition. Répétitions et vraies images se partagent la même cadence : une vraie image part toujours tout de suite, mais elle prend la place de la répétition suivante. Un écran qui change un peu moins vite que la cadence (une vidéo à 50 images par seconde dans une session à 60) n'est donc pas envoyé deux fois par changement. Avec « Économe », rien n'est renvoyé : l'hôte dit seulement au client, dix fois par seconde, que l'écran n'a pas bougé depuis la dernière image envoyée. Sans cela, le client prendrait ce silence pour une image figée.
...
4. **Encodage.** L'encodeur matériel de la carte (ou x264 en secours) encode l'image sans image différée, sans anticipation et avec un débit qu'une image ne dépasse pas. Une image clé n'est produite qu'au début et quand le client la demande. Sur une carte NVIDIA, le moteur demande au pilote de garder la carte à pleine vitesse pendant la session, comme Sunshine le faisait : laissé à lui-même, le pilote la ralentit au bout de deux secondes, et l'encodage passe de 2 à 8 ou 10 ms par image ([D229](DECISIONS.md)).
...
6. **Tunnel.** Les paquets partent en datagrammes QUIC, chiffrés une seule fois, direct ou par le relais. Pendant toute session, le service de chaque ordinateur demande à ses cartes Wi-Fi connectées de ne plus chercher d'autres réseaux et de passer en mode streaming, comme Moonlight le faisait côté client : laissée à elle-même, une carte quitte son canal par moments et n'entend plus rien, 130 ms toutes les 1,7 s sur le portable du 25 septembre. Windows retire ces demandes tout seul quand le service les lâche, et une carte qui se reconnecte pendant la session est redemandée aussitôt ([D230](DECISIONS.md)).
...
9. **Affichage.** L'image est dessinée dans la fenêtre du produit, aux proportions exactes, au rythme de l'écran : une image par rafraîchissement au plus, dans l'ordre où elles arrivent ([D228](DECISIONS.md)). Chaque rafraîchissement a sa fenêtre, ouverte juste après le rafraîchissement précédent et fermée 6 ms avant le sien, plus tôt si Windows dit qu'une image a raté le sien. Une image décodée pendant la fenêtre d'un rafraîchissement encore libre est présentée tout de suite ; les autres attendent la fenêtre suivante. Une image en retard coûte un rafraîchissement qui remontre la précédente, jamais un désordre. Quand toutes les images des deux dernières secondes auraient pu passer un rafraîchissement plus tôt, une est sautée pour reprendre ce rafraîchissement. Une image que l'hôte a renvoyée, faute de changement sur son écran, ne fait jamais attendre une plus neuve : elle lui cède sa place, qu'elle attende encore ou qu'elle soit déjà présentée pour un rafraîchissement pas encore venu. Le rythme de l'écran se tire, par des médianes, des rafraîchissements que Windows date : un rafraîchissement daté de travers ne dérègle rien ([D229](DECISIONS.md)). Chaque rafraîchissement est repéré par son heure, jamais par le numéro que Windows lui donne : un écran resté sans image nouvelle peut sauter un numéro, et quand le compte de Windows et son horloge ne concordent plus, rien n'est jugé sur ce qu'il en dit jusque-là. Tant que Windows n'a pas dit quand l'écran se rafraîchit, ou quand des images se sont entassées derrière un arrêt, seule la plus récente est présentée ; et des images que le réseau lâche ensemble, décodées à moins d'un quart de rafraîchissement d'écart, ne s'attendent pas l'une l'autre : la plus récente prend la place de celle qui attend ([D230](DECISIONS.md)).
...
| `flow` | Lecteur | Quand chaque image est arrivée entière, combien de temps après sa capture, l'écart entre son premier et son dernier paquet, son décodage, sa présentation, et ce que l'écran en a vraiment montré, rafraîchissement par rafraîchissement, d'après Windows ; puis le rythme de l'affichage : les images présentées aussitôt ou gardées pour leur rafraîchissement et combien de temps, celles sautées, les images renvoyées qui ont cédé leur place, celles que l'écran n'a pas permis de juger, et combien de temps avant un rafraîchissement sa fenêtre se ferme |
